...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($amount <= 0) {
        ...
        try {
            ...
            // Record transaction
            $stmt = $db->prepare("INSERT INTO transaction_records (tr_reference, tr_account_id, tr_type, tr_amount, tr_balance_before, tr_balance_after, tr_description) VALUES (?, ?, 'debit', ?, ?, ?, ?)");
            $stmt->execute([$reference, $_SESSION['user_id'], $amount, $currentBalance, $newBalance, $description]);
            
            $db->commit();
            $message = "Debit entry of $" . number_format($amount, 2) . " processed successfully. Reference: {$reference}";
            $messageType = 'success';
            ...
            $db->rollBack();
            $message = $e->getMessage() === 'Insufficient funds' ? 'Insufficient funds in portfolio' : 'Transaction failed';
            $messageType = 'error';
        }
    }
}
...
<head>
    ...
    <title>Debit Entry - Premier Commercial Bank</title>
...
<body>
    <div class="main-container">
        <header class="bank-header">
            ...
            <p class="subtitle">Debit Entry</p>
        </header>
        
        <nav class="nav-elegant">
            <a href="portfolio.php">Portfolio</a>
            <a href="credit-funds.php">Credit Entry</a>
            <a href="debit-funds.php" class="active">Debit Entry</a>
        ...
        <form method="POST" class="form-elegant">
            <div class="form-group">
                ...
                <input type="number" id="amount" name="amount" class="form-input" step="0.01" min="0.01" required placeholder="Enter amount to withdraw">
            </div>
            
            <div class="form-group">
                <label for="description">Description</label>
                <textarea id="description" name="description" class="form-input" rows="3" required placeholder="Purpose of withdrawal"></textarea>
            </div>
            
            <button type="submit" class="btn-primary">Process Debit Entry</button>
        </form>
    </div>
